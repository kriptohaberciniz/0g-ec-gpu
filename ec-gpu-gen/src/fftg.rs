...
use ark_ff::Field;
use ec_gpu::{GpuCurveAffine, GpuName};
use log::{error, info};
...
impl<'a, G: GpuCurveAffine> SingleFftGKernel<'a, G>
where
    G::Scalar: Field + GpuName,
{
...
pub struct FftGKernel<'a, G>
where
    G: GpuCurveAffine,
    G::Scalar: Field + GpuName,
{
    kernels: Vec<SingleFftGKernel<'a, G>>,
}

impl<'a, G> FftGKernel<'a, G>
where
    G: GpuCurveAffine,
    G::Scalar: Field + GpuName,
{
    ...
    fn create_optional_abort(
        ...
        let kernels: Vec<_> = programs
            ...
            .filter_map(|program| {
                ...
                let kernel = SingleFftGKernel::<G>::create(program, maybe_abort);
    ...
    pub fn radix_fftg(
        ...
        self.kernels[0].radix_fftg(input, omega, log_n)
    }
                ...
                .zip(self.kernels.iter_mut())
            {
                ...
                s.execute(move || {
                        ...
                        inputs.iter_mut().zip(omegas.iter()).zip(log_ns.iter())
                    {
                        ...
                        if let Err(err) = kern.radix_fftg(input, omega, *log_n) {
