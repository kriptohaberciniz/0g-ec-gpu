...
use ark_bn254::{G1Affine, Fr as Scalar};
use ark_std::UniformRand;
use ag_types::PrimeFieldRepr;
use criterion::{black_box, criterion_group, criterion_main, BenchmarkId, Criterion};
use ec_gpu_proxy::{
    multiexp::MultiexpKernel, multiexp_cpu::SourceBuilder, 
    threadpool::Worker,
};
...
fn bench_multiexp(crit: &mut Criterion) {
    ...
    let mut kern =
        MultiexpKernel::<G1Affine>::create(programs, &devices)
    ...
    let num_elements: Vec<_> = (16..MAX_ELEMENTS_POWER).map(|shift| 1 << shift).collect();
