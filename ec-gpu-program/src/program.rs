...
///
/// It will embed the CUDA fatbin/OpenCL source code within your binary. The
/// source needs to be generated via [`crate::source::generate`] in your
/// `build.rs`.
///
...
macro_rules! program {
    ($device:ident) => {{
        ...
        match check_framework($device) {
            #[cfg(feature = "cuda")]
            Ok(Framework::Cuda) => build_cuda_program(
                $device,
                include_bytes!(env!("_EC_GPU_CUDA_KERNEL_FATBIN")),
            ),
            #[cfg(feature = "opencl")]
            Ok(Framework::Opencl) => build_opencl_program(
                $device,
                include_str!(env!("_EC_GPU_OPENCL_KERNEL_SOURCE")),
            ),
            Err(e) => Err(e),
        }
    }};
        ...
        match check_framework($device) {
            ...
            Ok(Framework::Cuda) => {
                let mut file = std::fs::File::open(
                    std::env::var("_EC_GPU_CUDA_KERNEL_FATBIN").unwrap(),
                )
                .unwrap();
            ...
            #[cfg(feature = "opencl")]
            Ok(Framework::Opencl) => {
                let mut buffer = std::fs::read_to_string(
                    std::env::var("_EC_GPU_OPENCL_KERNEL_SOURCE").unwrap(),
                )
                .unwrap();
...
pub fn check_framework(device: &Device) -> EcResult<Framework> {
    // Selects a CUDA or OpenCL on the `EC_GPU_FRAMEWORK` environment variable
    // and the compile-time features.
    //
    // You cannot select CUDA if the library was compiled without support for
    // it.
...
#[cfg(feature = "cuda")]
pub fn build_cuda_program(
    device: &Device, kernel: &[u8],
) -> EcResult<rust_gpu_tools::Program> {
...
#[cfg(feature = "opencl")]
pub fn build_opencl_program(
    device: &Device, source: &str,
) -> EcResult<rust_gpu_tools::Program> {
    use rust_gpu_tools::{opencl::Program, GPUError};

    let opencl_device =
        device.opencl_device().ok_or(GPUError::DeviceNotFound)?;
